...
import com.master.chat.api.openai.entity.chat.FunctionCall;
...
public abstract class PluginListener<R extends PluginParam, T> extends EventSourceListener {
    ...
    public void onClosedAfter() {
        ...
        FunctionCall functionCall = FunctionCall.builder()
                ...
                .build();
        chatCompletion.getMessages().add(OpenAiMessage.builder().role(OpenAiMessage.Role.ASSISTANT).content("function_call").functionCall(functionCall).build());
        chatCompletion.getMessages().add(OpenAiMessage.builder().role(OpenAiMessage.Role.FUNCTION).name(plugin.getFunction()).content(plugin.content(tq)).build());
        //设置第二次，请求的参数
