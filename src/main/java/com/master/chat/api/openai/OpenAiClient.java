...
public class OpenAiClient {
     ...
     * @return Item  list
     */
    public List<ImageItem> editImages(java.io.File image, String prompt) {
     ...
     * @return Item  list
     */
    public List<ImageItem> editImages(java.io.File image, ImageEdit imageEdit) {
        return this.editImages(image, null, imageEdit);
    }
     ...
     * @return Item list
     */
    public List<ImageItem> editImages(java.io.File image, java.io.File mask, ImageEdit imageEdit) {
        checkImage(image);
...
//    }
     ...
     * 简易版（不支持图片输入）
     *
     * @param openAiMessages 问答参数
     * @return 答案
     */
    public ChatCompletionResponse chatCompletion(List<OpenAiMessage> openAiMessages) {
        ChatCompletion chatCompletion = ChatCompletion.builder().messages(openAiMessages).build();
    ...
    public <R extends PluginParam, T> ChatCompletionResponse chatCompletionWithPlugin(ChatCompletion chatCompletion, PluginAbstract<R, T> plugin) {
        ...
        FunctionCall functionCall = FunctionCall.builder()
                ...
                .name(plugin.getFunction())
                .build();
        openAiMessages.add(OpenAiMessage.builder().role(OpenAiMessage.Role.ASSISTANT).content("function_call").functionCall(functionCall).build());
        openAiMessages.add(OpenAiMessage.builder().role(OpenAiMessage.Role.FUNCTION).name(plugin.getFunction()).content(plugin.content(tq)).build());
        //设置第二次，请求的参数
     ...
     * @return ChatCompletionResponse
     */
    public <R extends PluginParam, T> ChatCompletionResponse chatCompletionWithPlugin(List<OpenAiMessage> openAiMessages, PluginAbstract<R, T> plugin) {
        return chatCompletionWithPlugin(openAiMessages, ChatCompletion.Model.GPT_3_5_TURBO_16K_0613.getName(), plugin);
    }
     ...
     * @return ChatCompletionResponse
     */
    public <R extends PluginParam, T> ChatCompletionResponse chatCompletionWithPlugin(List<OpenAiMessage> openAiMessages, String model, PluginAbstract<R, T> plugin) {
        ChatCompletion chatCompletion = ChatCompletion.builder().messages(openAiMessages).model(model).build();
     ...
     * 创建助手
     *
     * @param gptAssistant 参数
     * @return 返回助手信息
     * @since 1.1.2
     */
    public AssistantResponse assistant(GptAssistant gptAssistant) {
        Single<AssistantResponse> assistantResponse = this.openAiApi.assistant(gptAssistant);
     ...
     *
     * @param assistantId 助手id
     * @param gptAssistant   修改助手参数
     * @return 助手信息
     * @since 1.1.3
     */
    public AssistantResponse modifyAssistant(String assistantId, GptAssistant gptAssistant) {
        Single<AssistantResponse> assistantResponse = this.openAiApi.modifyAssistant(assistantId, gptAssistant);
        return assistantResponse.blockingGet();
    }
