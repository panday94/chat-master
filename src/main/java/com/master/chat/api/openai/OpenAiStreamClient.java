...
import com.master.chat.api.openai.entity.chat.BaseChatCompletion;
import com.master.chat.api.openai.entity.chat.ChatCompletion;
import com.master.chat.api.openai.entity.chat.Functions;
...
import com.master.chat.api.openai.sse.ConsoleEventSourceListener;
import com.master.chat.api.openai.sse.DefaultPluginListener;
import com.master.chat.api.openai.sse.PluginListener;
...
public class OpenAiStreamClient {
     ...
     * 警告：（不支持图片输入）
     *
     * @param openAiMessages            问答列表
     * @param eventSourceListener sse监听器
     * @see ConsoleEventSourceListener
     */
    public void streamChatCompletion(List<OpenAiMessage> openAiMessages, EventSourceListener eventSourceListener) {
     ...
     * @param <T>                 插件自定义函数的返回值
     */
    public <R extends PluginParam, T> void streamChatCompletionWithPlugin(List<OpenAiMessage> openAiMessages, EventSourceListener eventSourceListener, PluginAbstract<R, T> plugin) {
        this.streamChatCompletionWithPlugin(openAiMessages, ChatCompletion.Model.GPT_3_5_TURBO_16K_0613.getName(), eventSourceListener, plugin);
    }
     ...
     * @param <T>                 插件自定义函数的返回值
     */
    public <R extends PluginParam, T> void streamChatCompletionWithPlugin(List<OpenAiMessage> openAiMessages, String model, EventSourceListener eventSourceListener, PluginAbstract<R, T> plugin) {
