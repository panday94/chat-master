  ...
  methods: {
    ...
    getRoleMenuTreeselect(roleId) {
      return roleMenuTreeselect(roleId).then((response) => {
        this.menuOptions = response.data.routers;
        return response;
      });
    },
    /** 根据角色ID查询菜单树结构 */
    getRoleResourceIds(roleId) {
      return roleResourceIds(roleId).then((response) => {
        return response;
      });
    },
    /** 根据角色ID查询部门树结构 */
    ...
    handleUpdate(row) {
      ...
      const roleMenu = this.getRoleResourceIds(roleId);
    ...
    handleAuthUser: function (row) {
      const roleId = row.id;
      this.$router.push("/sys/role-auth/user/" + roleId);
    },
  },
};
</script>